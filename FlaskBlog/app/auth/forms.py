...
def email_exists(form, field):
    user = get_record('auth', Query().email == field.data)
    if not user:
        raise ValidationError('Please verify that you typed your email correctly')
...
def has_digits(form, field):
    if not bool(re.search(r'\d', field.data)):
        raise ValidationError('Your password must contains at least one number')
...
class LoginForm(Form):
    email = StringField('Email:', validators=[DataRequired(), Email(), account_exists])
    password = PasswordField('Password:', validators=[DataRequired(), PasswordCorrect('email')])
    submit = SubmitField('Log in')


class RegistrationForm(Form):
    email = StringField('Enter email address:', validators=[DataRequired(), Email()])
    password = PasswordField('Enter password: ' + '(min 12 char., must incl. number and special character)',
