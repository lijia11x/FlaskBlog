...
def browser_all_entries():
    ...
    return render_template('home.html', entries_for_page=entries_for_page, form=form, details=details,
                           next_page=next_page)
...
def view_entries_for_page(page):
    ...
    page = int(page)
    if page == 1:
        return redirect(url_for('main.browser_all_entries'))
    details = get_details()
    form = RawEntryForm()
    if form.validate_on_submit():
        return parse_input(form.raw_entry.data, datetime.utcnow())
    ...
    next_page = check_next_page(page)
    prev_page = page - 1
    return render_template('page.html', form=form, entries_for_page=entries_for_page, details=details, page=page,
...
def view_entries_for_day(day):
    ...
    if not entries_for_day:
        return abort(404)
    return render_template('day.html', form=form, day=day, entries_for_day=entries_for_day, details=details)
...
def view_single_entry(timestamp):
    ...
    if form.validate_on_submit():
        return parse_input(form.raw_entry.data, datetime.utcnow())
    details = get_details()
    return render_template('entry.html', form=form, timestamp=timestamp, entry=entry, details=details)
...
def edit_entry(timestamp):
    ...
    if form.validate_on_submit():
        ...
        update_record('entries', {'entry': form.new_entry.data, 'tags': tags},
                      (Query().creator_id == 1) & (Query().timestamp == timestamp))
        flash('Entry updated.')
        update_pagination()
    ...
    form.process()
    details = get_details()
    return render_template('edit_entry.html', form=form, timestamp=timestamp, details=details)
...
def view_all_tags():
    ...
    if form.validate_on_submit():
        return parse_input(form.raw_entry.data, datetime.utcnow())
    all_entries = search_records('entries', Query().creator_id == session.get('user_id'))
    all_tags = list()
    for entry in all_entries:
        for tag in entry['tags']:
            if tag not in all_tags:
                all_tags.append(tag)
    all_tags.sort()
    return render_template('tags.html', all_tags=all_tags, form=form,details=details)
