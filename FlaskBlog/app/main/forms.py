...
def use_commas(form, field):
    ...
    if items != 1:
        ...
        print(count)
        if count < items:
            raise ValidationError(Markup('Separate tags with commas like this: <code>tag1, tag2</code>.'))


class EditEntryForm(Form):
    new_entry = StringField('Edit entry text:', validators=[DataRequired()])
    new_tags = StringField("Edit tags: (separate with commas, don't use #)", validators=[no_hashtags, use_commas])
